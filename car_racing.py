...
class CarRacing(gym.Env, EzPickle):
    ...
    def _create_track(self):
        ...
        while 1:
            ...
            prev_beta = beta
            proj *= SCALE
            if proj >  0.3: beta -= min(self.turn_rate_factor * TRACK_TURN_RATE, abs(0.001*proj))
            if proj < -0.3: beta += min(self.turn_rate_factor * TRACK_TURN_RATE, abs(0.001*proj))
        ...
        for i in range(len(track)):
            ...
            road1_l = (x1 - self.track_width_factor*TRACK_WIDTH*math.cos(beta1), y1 - self.track_width_factor*TRACK_WIDTH*math.sin(beta1))
            road1_r = (x1 + self.track_width_factor*TRACK_WIDTH*math.cos(beta1), y1 + self.track_width_factor*TRACK_WIDTH*math.sin(beta1))
            road2_l = (x2 - self.track_width_factor*TRACK_WIDTH*math.cos(beta2), y2 - self.track_width_factor*TRACK_WIDTH*math.sin(beta2))
            road2_r = (x2 + self.track_width_factor*TRACK_WIDTH*math.cos(beta2), y2 + self.track_width_factor*TRACK_WIDTH*math.sin(beta2))
    ...
    def render_road(self):
        ...
        new_road_start_index = self.road_start_index
        for i in range(max(0, self.road_start_index - 1), len(self.road)):
            if self.road[i].road_visited:
                new_road_start_index = max(new_road_start_index, i)
        ...
        for i in range(self.road_start_index, new_road_start_index):
            self.road[i].road_visited = True
        self.road_start_index = new_road_start_index
